<template>
  <div class="j-col" :style="colStyle" :class="colClass">
    <slot></slot>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    colStyle() {
      return `padding: 0 ${this.interval / 2}px`;
    }
  },
  methods: {
    createClass(equipmentType, obj) {
      if (!obj) {
        return [];
      }
      let { span, offset } = obj;
      let arr = [];
      span && arr.push(`col-${equipmentType}${span}`);
      offset && arr.push(`col-${equipmentType}${offset}`);
      return arr;
    }
  }
};
